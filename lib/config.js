...
const SECRETS_PATH = path.join(ROOT, "agentic-os.secrets.json");
...
/** Read the gitignored secrets file ({ "<name>": "<api key>" }). */
...
/** Merge-and-save a single secret without exposing the others. */
export async function writeSecret(name, value) {
  const current = await readSecrets();
  if (value) current[name] = value;
  else delete current[name];
  await fs.writeFile(SECRETS_PATH, JSON.stringify(current, null, 2) + "\n", "utf8");
  return Object.keys(current);
}

/** The API key for a given API-mode agent, or null. */
export async function secretForAgent(agent) {
  if (!agent?.secret) return null;
  const secrets = await readSecrets();
  return secrets[agent.secret] || null;
}
